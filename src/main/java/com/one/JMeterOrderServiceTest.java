...
public class JMeterOrderServiceTest extends AbstractJavaSamplerClient {

    private long start = 0;//记录测试开始时间；
    private long end = 0;//记录测试结束时间；

    private String APP_NAME = "order-center";
    private String URL = "";
...
//        SERVICE_NAME = javaSamplerContext.getParameter("SERVICE_NAME");
    ...
    public Arguments getDefaultParameters() {
        ...
        arguments.addArgument("接口名(必填)", "com.one.order.PreOrderSearchService");
        arguments.addArgument("方法名(必填)", "searchPreOrder");
        arguments.addArgument("方法参数类型数组(必填)", "['com.one.order.bean.request.SearchOrderReq']");
